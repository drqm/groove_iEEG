...
blocks['wanting_to_move']['instr'] =  visual.TextStim(win, 
                text = 'Du skal nå få høre forskjellige korte musikkutdrag.\n\n'
                'Etter hvert utdrag vil du bli spurt om å rangere hvor mye musikken '
                'ga deg LYST TIL Å BEVEGE DEG, på følgende måte:\n\n '
                'Ikke i det hele tatt  < 1    2    3    4    5 >   veldig mye\n\n'
                'For å svare, trykk vennligst 1, 2, 3, 4, 5 på tastaturet.\n\n'
                'Trykk på mellomromstasten for å fortsette.',
                color=txt_color, wrapWidth=1.8)

blocks['pleasure']['instr'] =  visual.TextStim(win, 
                text = 'Du skal nå få høre forskjellige korte musikkutdrag.\n\n'
                'Etter hvert utdrag vil du bli spurt om å rangere '
                'HVOR MYE DU LIKTE musikken , på følgende måte:\n\n'
                'Ikke i det hele tatt  < 1    2    3    4    5 >   veldig mye\n\n'
                'For å svare, trykk vennligst 1, 2, 3, 4, 5 på tastaturet.\n\n'
                'Trykk på mellomromstasten for å fortsette.',
                color=txt_color, wrapWidth=1.8)
...
blocks['pleasure']['ratingtxt'] = pleasure_txt = visual.TextStim(win, 
                text = 'Hvor mye likte du musikken?\n\n'
                       'Ikke i det hele tatt  < 1    2    3    4    5 >   veldig mye\n\n',
                color=txt_color, wrapWidth=1.8)

blocks['wanting_to_move']['ratingtxt'] = visual.TextStim(win, 
                text = 'Hvor lyst hadde du til å bevege deg?\n\n'
                      'Ikke i det hele tatt  < 1    2    3    4    5 >   veldig mye\n\n',
                color=txt_color, wrapWidth=1.8)

practice = visual.TextStim(win, 
                text = 'Først, la oss prøve noen eksempler.\n\n'
                    'Når du er klar, trykk på mellomromstasten for å høre det første musikkutdraget.',
                color=txt_color, wrapWidth=1.8)

main_task = visual.TextStim(win,
                text = 'Dette er slutten på eksemplene.\n\n'
                    'Når du er klar, trykk på mellomromstasten for å starte forsøket.',
                color=txt_color, wrapWidth=1.8)

break_txt = visual.TextStim(win,
                text = 'Det er nå tid for en liten pause.\n'
                        'Ta så mye tid du trenger.\n\n'
                        'Trykk på mellomromstasten når du er klar til å fortsette.',
                color=txt_color, wrapWidth=1.8)

block_end_txt = visual.TextStim(win, 
                text = 'Det var slutten på den første delen.\n\n'
                        'Ta en liten pause, og trykk mellomromstasten når du er klar til å fortsette.',
                color=txt_color, wrapWidth=1.8)

end_txt = visual.TextStim(win, 
                text = 'Det var slutten på forsøket.\n'
...
log_fn_def = 'logs/' + sub_id[0] +  '_default_norwegian.log'
lastLog = logging.LogFile(log_fn_def, level=logging.INFO, filemode='a')

# set custom log file
log_fn_cus = 'logs/' + sub_id[0] +  '_custom_norwegian.csv'
...
def block_run(s_dict, s_order, b_sounds, breaks=[]):
    ...
    for mtrial, midx in enumerate(s_order): # loop over trials
        ...
        trialtxt.setText('Runde {} / {}'.format(mtrial + 1, len(s_order)))
        trialtxt.draw()
        #fixation.draw()
        win.flip()
        core.wait(1)
